...
import { fetchArticles } from '../lib/feed/article-fetcher';
import type { Article, StoredSummary, RankedArticle, DailyDigest } from '../lib/storage/storage-interface';
 ...
 */
async function generateArticleSummary(article: Article): Promise<string> {
 ...
 */
async function generateNarrativeSummary(articles: Article[]): Promise<string> {
  const apiKey = process.env.CLAUDE_API_KEY;
 ...
 */
async function generateHeadline(articles: Article[]): Promise<string> {
  const apiKey = process.env.CLAUDE_API_KEY;
 ...
 */
function generateTemplateHeadline(articles: Article[]): string {
 ...
 */
function generateTemplateSummary(articles: Article[]): string {
...
async function generateDigest() {
  ...
  try {
    ...
    console.log(`Digest saved to: data/digests/${today}.json`);
